...
def process_binary_file(input_filename, output_filename, start_offset, jmp_offset, entities_num):
    ...
    with open(input_filename, 'rb') as infile, open(output_filename, 'wb') as outfile:
        ...
        for _ in range(entities_num):
            ...
            # Print the address, original value, and multiplied value
...
def process_multiple_files(config_filename):
    with open(config_filename, 'r') as config_file:
        for line in config_file:
            ...
            process_binary_file(input_filename, output_filename, start_offset, jmp_offset, entities_num)

def main():
    parser = argparse.ArgumentParser(description='Process and edit binary files.')
    parser.add_argument('--filelist', type=str, help='The name of the configuration file with processing details.')
    ...
    if args.filelist:
        process_multiple_files(args.filelist)
        ...
        process_binary_file(args.input_filename, args.output_filename, args.start_offset, args.jmp_offset, args.entities_num)
    else:
        parser.print_help()
        sys.exit(1)
